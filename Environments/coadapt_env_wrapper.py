...
class CoadaptEnv:
    ...
    def __init__(self, config = {'env' : {'render' : True, 'record_video': False}}):
        ...
        env = gym.make(self._config['env']['env_name'], render_mode='rgb_array')

        self.trigger = lambda _: self._record_video
        self._env = gym.wrappers.RecordVideo(env=env, video_folder=self._config['data_folder_experiment'],
                                             episode_trigger=self.trigger)
        ...
        # ]
        self.observation_space = Box(-np.inf, np.inf, shape=[self._env.observation_space.shape[0] + 6], dtype=np.float32)#env.observation_space
        ...
        self._design_dims = list(range(self.observation_space.shape[0] - len(self._env.design), self.observation_space.shape[0]))
    ...
    def get_current_design(self):
        return copy.copy(self._env.design)
