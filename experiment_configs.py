...
#     'data_folder': '/scratch/adam/data_exp_sac_pso_batch',   # Name of the folder in which data generated during the experiments are saved
...
#     'rl_algorithm_config': dict(               # Dictionary which contains the parameters for the RL algorithm
...
#         copy_from_global=True,               # Shall we pre-initialize the individual network with the global network?
...
#         env_name='HalfCheetahCA-v0',             # Name of environment
...
sac_pso_batch = {
    ...
    'nmbr_random_designs': 0,                  # Number of random designs to run after the initial design TODO: Not used atm
    'iterations_init': 16,                    # Number of episodes for all initial designs as provided by the environment class
    'iterations_random': 16,                  # Number of episodes for all random designs TODO: Not used atm
    'iterations': 16,                         # Number of episodes to run for all designs after the initial
    'design_cycles': 16,                       # Number of design adaptations after the initial designs
    ...
    'cuda_device': 0,                          # Which cuda device to use. Only relevant if you have more than one GPU
    'exploration_strategy': 'random',          # Type of design exploration to use - we do usually one greedy design optimization and one random selection of a design
    'design_optim_method': 'pso_batch_sp',        # Which design optimization method to use
    'steps_per_episodes': 16,                # Number of steps per episode
    'save_networks': True,                     # If True networks are checkpointed and saved for each design
    'rl_method': 'SoftActorCritic',            # Which reinforcement learning method to use.
    'rl_algorithm_config': dict(               # Dictionary which contains the parameters for the RL algorithm
        algo_params=dict(                           # Parameters for the RL learner for the individual networks
        ...
        net_size=20,                       # Number of neurons in hidden layer
        network_depth=3,                    # Number of hidden layers
        copy_from_global=True,               # Shall we pre-initialize the individual network with the global network?
        indiv_updates=16,                 # Number of training updates per episode for individual networks
        pop_updates=16,                    # Number of training updates per episode for population networks
        batch_size=256,                     # Batch size
    ),
    'env': dict(                           # Parameters and which environment to use
        env_name='HalfCheetahCA-v0',             # Name of environment
...
sac_pso_sim = {
    ...
    'data_folder': '/scratch/adam/data_exp_sac_pso_sim',
    'nmbr_random_designs': 0,
    'iterations_init': 30,
    'iterations': 30,
    'iterations_random': 10,
    'design_cycles': 15,
    ...
    'steps_per_episodes': 10,                # Number of steps per episode
    'save_networks': True,
